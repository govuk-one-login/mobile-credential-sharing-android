...
object MissingPrerequisitesMatchers {
    ...
    fun hasBluetoothState(matcher: Matcher<in BluetoothState>): Matcher<in MissingPrerequisite> =
        MissingPrerequisitesMatcher(matcher) {
            (it as? MissingPrerequisites.Bluetooth)?.state
        }

    fun hasCameraState(expected: CameraState) = hasCameraState(Matchers.equalTo(expected))

    fun hasCameraState(matcher: Matcher<in CameraState>): Matcher<in MissingPrerequisite> =
        MissingPrerequisitesMatcher(matcher) {
            (it as? MissingPrerequisites.Camera)?.state
        }

    fun hasLocationState(expected: LocationState) = hasLocationState(Matchers.equalTo(expected))

    fun hasLocationState(matcher: Matcher<in LocationState>): Matcher<in MissingPrerequisite> =
        MissingPrerequisitesMatcher(matcher) {
