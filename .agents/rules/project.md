...
All dependency versions are managed centrally in `gradle/libs.versions.toml`. Always reference this file when adding or updating dependencies — never hardcode versions in `build.gradle.kts` files.

## Coding Conventions

- Follow existing Detekt rules (`detekt.yml`) — `warningsAsErrors` is off but violations should be avoided
