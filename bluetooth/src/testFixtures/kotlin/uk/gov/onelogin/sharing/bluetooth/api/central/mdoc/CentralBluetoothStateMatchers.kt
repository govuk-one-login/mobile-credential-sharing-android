...
object CentralBluetoothStateMatchers {
    ...
    fun hasAddress(matcher: Matcher<in String>): Matcher<in CentralBluetoothState> =
        CentralBluetoothStateMatcher(
            matcher
        ) {
    ...
    fun hasSessionEnd(matcher: Matcher<in Boolean>): Matcher<in CentralBluetoothState> =
        CentralBluetoothStateMatcher(matcher) {
            (it as? CentralBluetoothState.Disconnected)?.isSessionEnd
        }

    fun hasUuid(expected: UUID) = hasUuid(equalTo(expected))

    fun hasUuid(matcher: Matcher<in UUID>): Matcher<in CentralBluetoothState.Message> =
        CentralBluetoothStateMatcher(matcher) {
            (it as? CentralBluetoothState.Message)?.uuid
        }

    fun hasValue(expected: ByteArray) = hasValue(equalTo(expected))

    fun hasValue(matcher: Matcher<in ByteArray>): Matcher<in CentralBluetoothState.Message> =
        CentralBluetoothStateMatcher(matcher) {
            (it as? CentralBluetoothState.Message)?.value
        }

    fun isBluetoothEnded(matcher: Matcher<in SessionEndStates>): Matcher<in CentralBluetoothState> =
        CentralBluetoothStateMatcher(matcher) {
    ...
    fun isError(expected: CentralBluetoothTransportError): Matcher<in CentralBluetoothState> =
