...
class TestReaderAuthLeafCertificateGenerator:
    ...
    def test_validity_not_before_matches_generator_property(
        ...
        assert leaf_certificates.not_valid_before_utc == expected.replace(
            tzinfo=timezone.utc
        )
    ...
    def test_authority_identifier_derived_from_root_public_key(
        ...
        leaf_certificate_extensions: Extensions
    ):
        authority = leaf_certificate_extensions.get_extension_for_class(
            AuthorityKeyIdentifier
        )
    ...
    def test_extended_key_usage_is_critical(
        ...
        extended_keys = leaf_certificate_extensions.get_extension_for_class(
            ExtendedKeyUsage
        )
    ...
    def test_invalid_leaf_certificate_has_no_subject_information_access(
        self,
        invalid_leaf_certificate: Certificate
    ):
        with raises(ExtensionNotFound) as exception:
            invalid_leaf_certificate.extensions.get_extension_for_class(
                SubjectInformationAccess
            )
    ...
    def test_valid_leaf_certificate_has_privacy_policy_url(
        self,
        valid_leaf_certificate: Certificate
    ):
