...
class CoseSignatureVerifierTest {
    private val verifier =
        CoseSignatureVerifier(CoseHeaderValidator(SystemLogger()))
    ...
    fun `verify succeeds with valid signature`() {
        ...
        val coseSign1 = CoseSign1(protectedHeader, buildEmptyMap(), payload, signature)

        verifier.verify(coseSign1, publicKey, payload, error)
    }
    ...
    fun `verify throws with wrong public key`() {
        ...
        val exception = assertThrows(VerificationResult.Failure::class.java) {
            verifier.verify(coseSign1, otherKey, payload, error)
        }
        assertThat(exception, hasError(error))
    }
}
