...
object MobileSecurityObjectMatchers {
    fun hasDeviceKeyInfo(
        expected: DeviceKeyInfo
    ) = hasDeviceKeyInfo(equalTo(expected))
    fun hasDeviceKeyInfo(
        matcher: Matcher<in DeviceKeyInfo>
    ): Matcher<in MobileSecurityObject> = MobileSecurityObjectMatcher(matcher) {
        it?.deviceKeyInfo
    }
    fun hasDigestAlgorithm(
        expected: String
    ) = hasDigestAlgorithm(equalTo(expected))
    fun hasDigestAlgorithm(
    ...
    fun hasValidityInfo(
        expected: ValidityInfo
    ) = hasValidityInfo(equalTo(expected))
    fun hasValidityInfo(
        matcher: Matcher<in ValidityInfo>
    ): Matcher<in MobileSecurityObject> = MobileSecurityObjectMatcher(matcher) {
        it?.validityInfo
    }
    fun hasValueDigests(
