...
object VerifiableDocumentMatchers {
    ...
    ): Matcher<in VerifiableDocument> = VerifiableDocumentMatcher(matcher) {
        it?.docType
    }

    fun hasIssuerSigned(
        expected: IssuerSigned
    ) = hasIssuerSigned(equalTo(expected))

    fun hasIssuerSigned(
        matcher: Matcher<in IssuerSigned>
    ): Matcher<in VerifiableDocument> = VerifiableDocumentMatcher(matcher) {
        it?.issuerSigned
    }
    ...
    ) = hasDeviceSigned(equalTo(expected))
    ...
    private class VerifiableDocumentWithPresentationMatcher<Type>(
        ...
        override fun matchesSafely(
            item: VerifiableDocument.WithPresentation?
        ): Boolean = matcher.matches(transformer(item))
    }
}